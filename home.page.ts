...
export class HomePage implements OnInit {
    ...
    setFields(){
        ...
        for(let field of this.data) {
            switch(field.type){
                ...
                case 'checkbox':{
                    this.fields.push(this.formBuilder.group({
                        options: this.createFieldOptions(field.options)
                    }));
                    break;
                }
                default: {
    ...
    createFieldOptions(options: OptionDetails[]): FormArray{
        let optionsFormArray = new FormArray([]);
        for(let option of options){
            optionsFormArray.push(this.formBuilder.group({
                value: option.value,
                status: new FormControl(option.status)
            }))
        };
    ...
    onSubmit(){ 
        ...
        if (this.myForm.invalid) {
            ...
            for(let control of this.fields.controls){
                console.warn(JSON.stringify(control.value))
            ...
            console.warn(JSON.stringify(this.myForm.value))
        }
    }
